...
export MODELS=~/abcd/workspace/mding/immune_cell_models/modified_models
export DATA=~/abcd/workspace/mding/abcd_data
export RESULTS=~/abcd/workspace/mding/abcd_imputed/modified_models
...
for filename in $MODELS/*.db; do
  ...
  --vcf_mode genotyped \
  --prediction_output $RESULTS/$fn/abcd_pred_${SLURM_ARRAY_TASK_ID}.txt  \
  --prediction_summary_output $RESULTS/$fn/summary_abcd_pred_${SLURM_ARRAY_TASK_ID}.txt \
  --verbosity 9 \
  --throw
done