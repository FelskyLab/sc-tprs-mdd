...
# run abcd_twas_dep.sh



# new tPRS w/ TWAS, p < 0.02 ===========================
...
# no, male, female, anx, with all weighted
#anxdep
...
# lin reg with anx/dep, with/dep as outcomes
# start with corr matr between dep scores and weighted 
...
# anxdep, withdep quartiles ===============================

base_white_tprs_q <- within(base_white_tprs, quartile_ad <- as.integer(cut(anxdep_t, unique(quantile(anxdep_t, probs=0:6/6)), include.lowest=T)))
base_white_tprs_q <- within(base_white_tprs_q, quartile_wd <- as.integer(cut(withdep_t, unique(quantile(withdep_t, probs=0:7/7)), include.lowest=T)))
quartile_ad_mat <- matrix(nrow = 4, ncol = 4)
quartile_wd_mat_norm <- quartile_ad_mat
quartile_ad_mat_norm <- quartile_ad_mat

for (i in 1:4){
  for (j in 1:4){
    quartile_ad_mat[i,j] <- sum(((base_white_tprs_q$quartile_ad == i) & (base_white_tprs_q$quartile_wd == j)))
  }
}

for (i in 1:4){
  for (j in 1:4){
    quartile_wd_mat_norm[i,j] <- quartile_ad_mat[i,j]/sum(quartile_ad_mat[,j])
    quartile_ad_mat_norm[i,j] <- quartile_ad_mat[i,j]/sum(quartile_ad_mat[i,])
  }
}
quartile_ad_mat

corrplot(quartile_ad_mat, is.corr = F, method = "color", addCoef.col = 'grey')
corrplot(quartile_ad_mat_norm, is.corr = F, method = "color", addCoef.col = 'grey')
corrplot(quartile_wd_mat_norm, is.corr = F, method = "color", addCoef.col = 'grey')
...
p <- ggplot(base_white_tprs, aes(x = anxdep_t, y = withdep_t), alpha = 0.6, shape = 16) +  
  scale_color_brewer(palette = "Dark2") + 
  theme_bw() +
  theme(legend.position = "bottom") + 
  labs(x = "anxdep", y = "withdep") +
  geom_density_2d_filled(alpha = 0.5) +
  geom_density_2d(linewidth = 0.25, colour = "black") +
  geom_point(alpha = 0)
p
ggMarginal(p, type = "density", fill = "orange")

for (i in 1:4){
  for (j in 1:4){
    quartile_ad_mat[i,j] <- sum(((base_white_tprs_q$quartile_ad[which(base_white_tprs_q$sex=="1")][which(inds_wd_m=="test")] == i) & 
                                   (base_white_tprs_q$quartile_wd[which(base_white_tprs_q$sex=="1")][which(inds_wd_m=="test")] == j)))
  }
}
...
ggplot(twas_wd_white_all_tprs[which(twas_wd_white_all_tprs$sex=="1"),][which(inds_wd_m=="test"),], aes(x=`effector_memory_CD8-positive_alpha-beta_T_cell_w_m`, y=withdep_t)) + 
  geom_point() + theme_stata()


ggplot(twas_ad_white_all_tprs[which(twas_ad_white_all_tprs$sex=="1"),][which(inds_ad_m=="train"),], aes(x=`natural_killer_w_m`, y=anxdep_t)) + 
  geom_point() + theme_stata()

ggplot(twas_ad_white_all_tprs[which(twas_ad_white_all_tprs$sex=="1"),][which(inds_ad_m=="test"),], aes(x=`natural_killer_w_m`, y=anxdep_t)) + 
  geom_point() + theme_stata()


ggplot(twas_ad_white_all_tprs[which(twas_ad_white_all_tprs$sex=="2"),][which(inds_ad_f=="train"),], 
       aes(x=`peripheral_blood_mononuclear_cell_w_f`, y=anxdep_t)) + 
  geom_point() + theme_stata()

ggplot(twas_ad_white_all_tprs[which(twas_ad_white_all_tprs$sex=="2"),][which(inds_ad_f=="test"),], 
       aes(x=`peripheral_blood_mononuclear_cell_w_f`, y=anxdep_t)) + 
  geom_point() + theme_stata()

ggplot(twas_ad_white_all_tprs[which(twas_ad_white_all_tprs$sex=="2"),][which(inds_ad_f=="train"),], 
       aes(x=`CD8-positive_alpha-beta_T_cell_w_f`, y=anxdep_t)) + 
  geom_point() + theme_stata()
...
ggplot(twas_ad_white_all_tprs[which(twas_ad_white_all_tprs$sex=="2"),][which(inds_ad_f=="test"),], 
       aes(x=`erythrocyte_w_f`, y=anxdep_t)) + 
  geom_point() + theme_stata()